...
export const authService = {
  ...
  async checkAuth() {
    return new BaseService().handleResponse(api.get('/auth/me'));
  },
  ...
  async changePassword(passwordData) {
    return new BaseService().handleResponse(
  ...
  async checkApiHealth() {
    try {
      ...
      try {
        await axios.get(API_CONFIG.baseURL, { timeout: 5000 });
        return { success: true };
      } catch (fallbackError) {
        return { success: false, message: 'API nicht erreichbar' };
      }
    }
  },
};
